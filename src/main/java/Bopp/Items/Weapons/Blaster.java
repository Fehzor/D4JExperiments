 ...
 * @author FF6EB4
 */
public class Blaster extends Gear {
    int num, rate;
    public Blaster(String name, String prereq,int stars, int chance, int rate){
        super(name,prereq,"",stars);
        this.name = name;
        this.stars = stars;

        super.description = "Give away "+chance+" random materials! Get "+rate+"x crowns from drops!";
        this.slot = WEAPON;
    ...
    public void effect(UserData UD, Gate G){
            ...
            if(UD.materials.data[ran] < num){
                return;
            }
            UserData.getUser(win).materials.data[ran] += num;
            UD.materials.data[ran] -= num;
            
            UD.crowns.data += (rate-1)*(100 + oRan.nextInt(1000));
    }
}
