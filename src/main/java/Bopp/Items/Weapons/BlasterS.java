 ...
 * @author FF6EB4
 */
public class BlasterS extends Gear {
    int num, rate;
    public BlasterS(String name, String prereq,int stars, int chance, int rate){
        super(name,prereq,"",stars);
        this.name = name;
        this.stars = stars;

        super.description = "+"+chance+"% chance to give away all crowns! Get "+rate+"x crowns from drops!";
        this.slot = WEAPON;
    ...
    public void effect(UserData UD, Gate G){
        if(oRan.nextInt(100)<num){
            ...
            UserData.getUser(win).crowns.data += UD.crowns.data;
            UD.crowns.data=0;
        }
