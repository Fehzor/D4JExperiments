 ...
 * @author FF6EB4
 */
public class CSaber extends Gear{
    public CSaber(String name, String prereq,int stars){
        super(name,prereq,"",stars);
        this.name = name;
        this.stars = stars;

        super.description = "+convert "+stars*3+" materials into "+stars*2+" slime coins!";
        this.slot = WEAPON;
    ...
    public void effect(UserData UD, Gate G){
        ...
        if(UD.materials.data[choice] < (stars*3)){
            return;
        }
        UD.materials.data[choice] -= stars*3;
        UD.slimeCoins.data += stars*2;
    }
}
