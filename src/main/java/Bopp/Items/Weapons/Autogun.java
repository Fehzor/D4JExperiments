 ...
 * @author FF6EB4
 */
public class Autogun extends Gear{
        int odds;
    public Autogun(String name, String prereq,int stars, int chance){
        super(name,prereq,"",stars);
        this.name = name;
        this.stars = stars;

        super.description = "+"+chance+"% chance of doubled crowns!";
        this.slot = WEAPON;
    ...
    public void effect(UserData UD, Gate G){
        if(oRan.nextInt(100) < odds){
            UD.crowns.data += oRan.nextInt(1000)+100;
        }
    }
}
