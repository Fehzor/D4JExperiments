 ...
 * @author FF6EB4
 */
public class Mixer extends Gear {
    int num;
    public Mixer(String name, String prereq,int stars, int chance){
        super(name,prereq,"",stars);
        this.name = name;
        this.stars = stars;

        super.description = "+Moves+duplicates "+chance+" crowns around randomly!";
        this.slot = WEAPON;
    ...
    public void effect(UserData UD, Gate G){
            ...
            if(UserData.getUser(win).crowns.data < num){
                return;
            }
            UserData.getUser(win).crowns.data -= num;
            UserData.getUser(los).crowns.data += num*2;
    }
}
