 ...
 * @author FF6EB4
 */
public class GRepeater extends Gear{
        int odds;
    public GRepeater(String name, String prereq,int stars, int chance){
        super(name,prereq,"",stars);
        this.name = name;
        this.stars = stars;

        super.description = "+"+chance+"% to trade "+(stars*3000)+" crowns for "
                +(stars+5)+" slime coins!";
        this.slot = WEAPON;
    ...
    public void effect(UserData UD, Gate G){
        if(oRan.nextInt(100) < odds){
            if(UD.crowns.data < stars*3000){
                return;
            }
            UD.crowns.data -= stars*3000;
            UD.slimeCoins.data += stars+5;
        }
    }
}
