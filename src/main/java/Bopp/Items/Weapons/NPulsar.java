 ...
 * @author FF6EB4
 */
public class NPulsar extends Gear {
    int odds;
    public NPulsar(String name, String prereq,int stars, int chance){
        super(name,prereq,"",stars);
        this.name = name;
        this.stars = stars;

        super.description = "+"+chance+"% chance of stealing orbs from others!";
        this.slot = WEAPON;
    ...
    public void effect(UserData UD, Gate G){
        if(oRan.nextInt(100) < odds){
            ...
            int ran = oRan.nextInt(4);
            if(UserData.getUser(win).orbs.data[ran] == 0){
                return;
            }
            UserData.getUser(win).orbs.data[ran] -= 1;
            UD.orbs.data[ran] += 1;
            
            G.send(UD.name.data+
                    " stole an orb from "+
                    UserData.getUser(win).name.data+"!");
        }
    }
}
