 ...
 * @author FF6EB4
 */
public class DPulsar extends Gear {
    int odds;
    public DPulsar(String name, String prereq,int stars, int chance){
        super(name,prereq,"",stars);
        this.name = name;
        this.stars = stars;

        super.description = "+"+chance+"% chance of stealing minerals from others!";
        this.slot = WEAPON;
    ...
    public void effect(UserData UD, Gate G){
        if(oRan.nextInt(100) < odds){
            ...
            if(UserData.getUser(win).minerals.data[ran] < 10){
                return;
            }
            UserData.getUser(win).minerals.data[ran] -= 10;
            UD.minerals.data[ran] += 10;
        }
    }
}
