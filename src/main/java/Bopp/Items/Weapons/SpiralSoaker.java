 ...
 * @author FF6EB4
 */
public class SpiralSoaker extends Gear{
        int odds;
    public SpiralSoaker(String name, String prereq,int stars, int chance){
        super(name,prereq,"",stars);
        this.name = name;
        this.stars = stars;

        super.description = "+"+chance+"% chance of 10x crowns!";
        this.slot = WEAPON;
    ...
    public void effect(UserData UD, Gate G){
        if(oRan.nextInt(100) < odds){
            UD.crowns.data += (oRan.nextInt(1000)+100)*9;
        }
    }
}
