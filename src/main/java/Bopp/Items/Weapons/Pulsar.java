 ...
 * @author FF6EB4
 */
public class Pulsar extends Gear {
    int odds;
    public Pulsar(String name, String prereq,int stars, int chance){
        super(name,prereq,"",stars);
        this.name = name;
        this.stars = stars;

        super.description = "+"+chance+"% chance of stealing mats from others!";
        this.slot = WEAPON;
    ...
    public void effect(UserData UD, Gate G){
        if(oRan.nextInt(100) < odds){
            ...
            if(UserData.getUser(win).materials.data[ran] < 5){
                return;
            }
            UserData.getUser(win).materials.data[ran] -= 5;
            UD.materials.data[ran] += 5;
        }
    }
}
