 ...
 * @author FF6EB4
 */
public class ScissorBlades extends Gear{
    public ScissorBlades(String name, String prereq,int stars){
        super(name,prereq,"",stars);
        this.name = name;
        this.stars = stars;

        super.description = "+unbox faster (no need to equip)! Also convert "+stars+" materials into slime coins!";
        this.slot = WEAPON;
    ...
    public void effect(UserData UD, Gate G){
        ...
        if(UD.materials.data[choice] < stars){
            return;
        }
        UD.materials.data[choice] -= stars;
