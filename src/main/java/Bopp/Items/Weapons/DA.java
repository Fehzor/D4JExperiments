 ...
 * @author FF6EB4
 */
public class DA extends Gear{
    int amt,odds;
    public DA(String name, String prereq,int stars, int amt, int chance){
        super(name,prereq,"",stars);
        this.name = name;
        this.stars = stars;

        super.description = "+"+chance+"% chance of a random user receiving "+amt+" slime coin(s)!";
        this.slot = WEAPON;
    ...
    public void effect(UserData UD, Gate G){
        if(oRan.nextInt(100) < odds){
            ...
            UserData.getUser(win).slimeCoins.data += amt;
        }
    }
}
