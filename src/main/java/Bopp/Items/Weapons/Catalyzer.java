 ...
 * @author FF6EB4
 */
public class Catalyzer extends Gear {
    int num;
    public Catalyzer(String name, String prereq,int stars, int chance){
        super(name,prereq,"",stars);
        this.name = name;
        this.stars = stars;

        super.description = "+Moves+duplicates "+chance+" materials around randomly!";
        this.slot = WEAPON;
    ...
    public void effect(UserData UD, Gate G){
        if(oRan.nextInt(100)<37){
            ...
            if(UserData.getUser(win).materials.data[ran] < num){
                return;
            }
            UserData.getUser(win).materials.data[ran] -= num;
            UserData.getUser(los).materials.data[ran] += num*2;
        }
