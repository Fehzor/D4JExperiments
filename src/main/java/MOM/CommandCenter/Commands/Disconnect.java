...
public class Disconnect extends Command{
    ...
    public Disconnect(){
        ...
        this.description = "Disconnect room# from room#- use cautiously (Thwackers)";
        this.category = 2;
    }
        ...
        if(UD.task.getData().equals("Nothing doing.")){
            try{
                ...
                if(A.connections.contains(b)){
                    UD.setTimer(45);
                    UD.task.writeData("Disconnecting "+a+" from "+b);
                    MoM.PM("Disconnecting...",UD);
                } else {
                    MoM.PM("Those rooms aren't connected...", UD);
                }
            } catch (Exception E){MoM.PM("Failed to disconnect room!", UD);};
        } else {
            MoM.PM("You want to disconnect rooms, but you're busy! Try !cancel", UD);
        }
